...
public:
    ...
    : Node("atr_state_list_listener")
    {
        ...
        // subscribe to the state list and create service for measurements
...
private:
    // Callback for measurement service
    ...
    std::shared_ptr<atr_interfaces::srv::StartMeasurementSrv::Response> response){
        // Store parameters and start measurement
    ...
    void topic_callback(const atr_interfaces::msg::ATRStateListStamped::SharedPtr msg)
    {
        ...
        if(measurement){
            ...
            if(samples >= max_samples){
                // Write Timestamp samples in log file
                measurement_idx++;
